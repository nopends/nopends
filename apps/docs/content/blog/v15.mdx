...
Nopends v15 has no other significant changes than migrating the config to Tailwind CSS v4.
...
Add `@import` to the presets exported by Nopends UI, and include `nopends-ui` package into source.
...
Nopends no longer use `--fd-<color>` like `--fd-primary: 0 0% 0%` for colors, it directly defines and uses colors in `@theme`.
...
v15 improved the playground UI (inspired by the minimalism of Scalar), and brought Scalar API Client support to Nopends OpenAPI.
...
<Callout>
  ...
  stylesheet of Nopends UI will conflict with their `style.css` because both
...
In the future, we also want to make further improvements to Nopends MDX:
