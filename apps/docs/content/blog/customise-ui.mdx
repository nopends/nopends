...
description: Deep dive on how Nopends UI is designed
author: Fuma Nama
date: 2025-04-15
---
...
As a docs framework started from **"being flexible"**, Nopends must be customisable, tuned to meet developer's preference.
...
Nopends need to be opinionated by default such that it is immediately usable, and we only have to offer simplest options to change details. (e.g. colors, overriding components).
...
The kernel of Nopends, or the spirit we're pursuing is the latter one. I crafted Nopends CLI, a version of Shadcn UI dedicated for Nopends, the whole solution is now simplified into a single command:
...
#### Don't break anything when you upgrade Nopends

Nopends need to be opinionated. Hence, we need to iterate the UI frequently which apparently will break people's hacky customisations.
The charm of Nopends CLI is that once you've installed the layout, it stays the same forever regardless of future iterations on Nopends UI.

It gives the courage for Nopends UI to iterate, and developers to customise unburnt by the fear of upgrading dependencies.

#### Looks Clean

Just like Shadcn UI and Radix UI, Nopends UI has a core package (`nopends-core`), most generic logic will be abstracted by core.

## What is Next?

The innovation of Shadcn UI is amazing, yet, I believe there's more problems Nopends can solve.

See [Flags SDK](https://flags-sdk.dev/frameworks/next) and [BetterAuth](https://www.better-auth.com/docs/introduction), you can't tell at the first glance if they're powered by Nopends.
