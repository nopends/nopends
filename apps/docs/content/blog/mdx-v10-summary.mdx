...
```ts
import createMDX from 'nopends-mdx/config';
...
Now you can generate types using `nopends-mdx` command, it's recommended to set it as a `postinstall` script.
...
Now to integrate it with Nopends framework, change your `source.ts` from:
...
You can read the latest docs of Nopends MDX for details.

<Cards>
  <Card href="/docs/mdx" title="Nopends MDX">
