---
title: Introduction
description: What is Nopends MDX?
icon: Album
---

Nopends MDX is the official content source of Nopends.
...
Setup Nopends MDX for your Next.js application.
...
<Callout title="ESM Only" type='warn' className="mt-4">
    ...
    The Next.js config must be a `.mjs` file since Nopends is ESM-only.
...
Nopends MDX transforms collections into arrays of type-safe data, accessible in your app, available collections:
...
Combination of `meta` and `doc` collections, which is needed for Nopends.
...
The `nopends-mdx` command also generates types for `.source` folder, add it as a post install script to ensure types are generated when initializing the project.
...
## Integrate with Nopends

Create a `docs` collection and use the `toNopendsSource()` function of its output.
