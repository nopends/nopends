...
Nopends provides many useful extensions to MDX, a markup language. Here is a brief introduction to the default MDX syntax of Nopends.

> MDX is not the only supported format of Nopends. In fact, you can use any renderers such as `next-mdx-remote` or CMS.
...
> This is only available on **Nopends MDX**.
...
```ts tab="Nopends MDX" title="source.config.ts"
...
import { remarkCodeTab } from 'nopends-core/mdx-plugins';
