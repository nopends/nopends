---
title: Themes
description: Add Theme to Nopends UI
---

## Usage

Only Tailwind CSS v4 is supported, the preset will also include source to Nopends UI itself:
...
Nopends supports light/dark modes with [`next-themes`](https://github.com/pacocoursey/next-themes), it is included in Root Provider.
...
Nopends UI adds its own colors, animations, and utilities with Tailwind CSS preset.
...
```css
@import 'nopends-ui/css/<theme>.css';
@import 'nopends-ui/css/preset.css';
```
...
```css
@import 'tailwindcss';
@import 'nopends-ui/css/shadcn.css';
@import 'nopends-ui/css/preset.css';
```
...
> The plugin works with and only with Nopends UI's MDX components, it may conflict with `@tailwindcss/typography`.
