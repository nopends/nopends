...
```tsx
import { DocsBody } from 'nopends-ui/page';
...
<DocsBody>
  <a
    href={`https://github.com/nopends/nopends/blob/main/${page.file.path}`}
...
```tsx
import { DocsPage } from 'nopends-ui/page';
...
[TOC Utility](/docs/headless/utils/get-toc). Content sources like Nopends MDX offer this out-of-the-box.
...
remote sources like Sanity, Nopends UI doesn't display the last updated time by
default.
...
```tsx
import { DocsPage } from 'nopends-ui/page';
import { getGithubLastEdit } from 'nopends-core/server';
