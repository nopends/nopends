...
Nopends Core offers server-side functions and headless components to build docs on any React.js frameworks like Next.js.
...
<Callout title="Tip">
    ...
    It can be used without Nopends UI, in other words, it's headless.

    For beginners and normal usages, use [Nopends UI](/docs/ui).
...
export function RootLayout({ children }: { children: ReactNode }) {
  // or if you're using Nopends UI, use `<RootProvider />`
