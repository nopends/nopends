...
**Nopends is very flexible.** You can integrate with any content source, even without an official adapter.

> This guide assumes you are experienced with Next.js App Router.

### Examples

You can see examples to use Nopends with a CMS, which allows a nice experience on publishing content, and real-time update without re-building the app.

- [BaseHub](https://github.com/nopends/nopends-basehub)
- [Sanity](https://github.com/nopends/nopends-sanity)
...
```tsx
import { DocsPage, DocsBody } from 'nopends-ui/page';
...
Nopends offers a simple [Algolia Search Adapter](/docs/headless/search/algolia), which includes a search client to integrate with Nopends UI.

## MDX Remote

Nopends offers the **MDX Remote** package, it is a helper to integrate Markdown-based content sources with Nopends.
You can think it as a `next-mdx-remote` with built-in plugins for Nopends.
...
```tsx
import { compileMDX } from '@nopends/mdx-remote';
import { getPage } from './my-content-source';
import { DocsBody, DocsPage } from 'nopends-ui/page';
