...
description: Use Content Collections for Nopends
---
...
```package-install
@nopends/content-collections @content-collections/core @content-collections/mdx @content-collections/next
```
...
To integrate with Nopends, add the following to your `content-collections.ts`.
...
import { transformMDX } from '@nopends/content-collections/configuration';
...
To use components from other packages like Nopends UI, pass them to your `<MDXContent />` component.
...
<Callout title='Deep Dive: Why?'>
    ...
    To support importing a package from node modules, Nopends added a default value to the `cwd` option of MDX Bundler.
