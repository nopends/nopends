...
<Callout title='Before reading'>
    ...
    This guide only applies for content sources that uses `loader()` API, such as Nopends MDX.

</Callout>

## Overview

While Next.js handles routing, Nopends generates **page slugs** and **sidebar items** (page tree) from your content directory using [`loader()`](/docs/headless/source-api).
...
| `full`        | Fill all available space on the page (Nopends UI) |

<Callout title='Nopends MDX'>
...
Since Nopends doesn't include an icon library, you have to convert the icon names to JSX elements in runtime, and render it as a component.
...
<Callout title='Nopends UI'>

    Nopends UI renders root folders as [Sidebar Tabs](/docs/ui/navigation/sidebar#sidebar-tabs), which allows user to switch between them.
